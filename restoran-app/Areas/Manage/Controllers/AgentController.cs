...
using restoran_app.Areas.Manage.Helpers.Extensions;
using restoran_app.DAL.Context;
using restoran_app.Models;

namespace restoran_app.Areas.Manage.Controllers;
[Area("Manage")]
...
public class AgentController : Controller
{
    ...
    public async Task<IActionResult> Create(CreateAgentDto agentDto)
    {
        ...
        {
            ModelState.AddModelError("File", "Enter the file type correctly");
            return View(agentDto);
        }

        if (agentDto.File.Length > 3_000_000)
        {
            ModelState.AddModelError("File", "File size cannot exceed 3Mb");
            return View(agentDto);
        }
